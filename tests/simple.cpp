...
TEST(FlexiC, SingleString)
{
    ...
    size_t len = 0;
    ASSERT_TRUE(flexi_cursor_get_string_len(&cursor, &len));
    ASSERT_EQ(len, 18);

    const char *ch = nullptr;
    ASSERT_TRUE(flexi_cursor_get_string(&cursor, &ch));
    ASSERT_STREQ(ch, "hello flexbuffers!");
}

TEST(FlexiC, SingleIndirectInt1)
{
