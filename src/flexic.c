...
#define SINT_WIDTH(v)                                                                                                  \
    ((v) <= INT8_MAX && (v) >= INT8_MIN      ? 1                                                                       \
     ...
     : (v) <= UINT32_MAX && (v) >= INT32_MIN ? 4                                                                       \
                                             : 8);
#define UINT_WIDTH(v) ((v) <= UINT8_MAX ? 1 : (v) <= UINT16_MAX ? 2 : (v) <= UINT32_MAX ? 4 : 8);
...
typedef enum _write_vector_iter_e
{
    WRITE_VECTOR_ITER,
...
{
    ASSERT(cursor->type == FLEXI_TYPE_STRING || cursor->type == FLEXI_TYPE_MAP || //
           cursor->type == FLEXI_TYPE_VECTOR || cursor->type == FLEXI_TYPE_VECTOR_SINT ||
           cursor->type == FLEXI_TYPE_VECTOR_UINT || cursor->type == FLEXI_TYPE_VECTOR_FLOAT ||
           cursor->type == FLEXI_TYPE_BLOB || cursor->type == FLEXI_TYPE_VECTOR_BOOL);

    uint64_t v;
    if (!read_uint_by_width(&v, cursor->cursor - cursor->stride, cursor->stride))
    {
        return false;
    }
...
static bool cursor_map_key_at_index(const flexi_cursor_s *cursor, size_t index, const char **str)
{
    ...
    if (!read_uint_by_width(&keys_offset, cursor->cursor - (cursor->stride * 3), cursor->stride))
    {
        return false;
    }

    if (!read_uint_by_width(&keys_width, cursor->cursor - (cursor->stride * 2), cursor->stride))
    {
        return false;
    }
 ...
 * @return True if lookup was successful.
 */
static bool cursor_seek_map_key_linear(const flexi_cursor_s *cursor, size_t len, const char *key, flexi_cursor_s *dest)
{
    ASSERT(cursor->type == FLEXI_TYPE_MAP);
 ...
 * @return True if lookup was successful.
 */
static bool cursor_seek_map_key_bsearch(const flexi_cursor_s *cursor, size_t len, const char *key, flexi_cursor_s *dest)
{
    ASSERT(cursor->type == FLEXI_TYPE_MAP);
...
static bool reader_emit_sint(const flexi_reader_s *reader, const flexi_cursor_s *cursor)
{
    int64_t u;
...
static bool reader_emit_uint(const flexi_reader_s *reader, const flexi_cursor_s *cursor)
{
    uint64_t u;
...
static bool reader_emit_float(const flexi_reader_s *reader, const flexi_cursor_s *cursor)
{
...
static bool reader_emit_string(const flexi_reader_s *reader, const flexi_cursor_s *cursor)
{
    size_t len;
    if (!cursor_get_len(cursor, &len))
    {
...
static bool reader_emit_map(const flexi_reader_s *reader, const flexi_cursor_s *cursor)
{
    size_t len;
    if (!cursor_get_len(cursor, &len))
    {
...
static bool reader_emit_vector(const flexi_reader_s *reader, const flexi_cursor_s *cursor)
{
    size_t len;
    if (!cursor_get_len(cursor, &len))
    {
...
static bool reader_emit_vector_sint(const flexi_reader_s *reader, const flexi_cursor_s *cursor)
{
    size_t len;
    if (!cursor_get_len(cursor, &len))
    {
...
static bool reader_emit_vector_uint(const flexi_reader_s *reader, const flexi_cursor_s *cursor)
{
    size_t len;
    if (!cursor_get_len(cursor, &len))
    {
...
static bool reader_emit_vector_float(const flexi_reader_s *reader, const flexi_cursor_s *cursor)
{
    size_t len;
    if (!cursor_get_len(cursor, &len))
    {
...
static bool reader_emit_vector_key(const flexi_reader_s *reader, const flexi_cursor_s *cursor)
{
    size_t len;
    if (!cursor_get_len(cursor, &len))
    {
...
static bool reader_emit_vector_blob(const flexi_reader_s *reader, const flexi_cursor_s *cursor)
{
    size_t len;
    if (!cursor_get_len(cursor, &len))
    {
...
static bool reader_emit_vector_bool(const flexi_reader_s *reader, const flexi_cursor_s *cursor)
{
    size_t len;
    if (!cursor_get_len(cursor, &len))
    {
 ...
 */
static flexi_value_s *writer_get_inline_key(flexi_writer_s *writer, int start, int index)
{
...
static bool write_vector_values(flexi_writer_s *writer, size_t len, int stride, write_vector_iter_e iter)
{
...
static bool write_vector_types(flexi_writer_s *writer, size_t len)
{
    for (int i = 0; i < (int)len; i++)
    {
        ...
        flexi_packed_t packed = PACK_TYPE(value->type) | PACK_WIDTH(value->width);
...
bool flexi_cursor_bool(const flexi_cursor_s *cursor, bool *v)
{
    ...
    else if (cursor->type == FLEXI_TYPE_FLOAT && cursor->stride == sizeof(float))
    {
        float u;
        if (!read_f32(&u, cursor->cursor, cursor->stride))
        {
    ...
    else if (cursor->type == FLEXI_TYPE_FLOAT && cursor->stride == sizeof(double))
    {
        double u;
        if (!read_f64(&u, cursor->cursor, cursor->stride))
        {
...
bool flexi_read(const flexi_reader_s *reader, const flexi_cursor_s *cursor)
{
    ...
    case FLEXI_TYPE_FLOAT:
    case FLEXI_TYPE_INDIRECT_FLOAT: return reader_emit_float(reader, cursor);
    ...
    case FLEXI_TYPE_VECTOR_FLOAT: return reader_emit_vector_float(reader, cursor);
    case FLEXI_TYPE_VECTOR_KEY: return reader_emit_vector_key(reader, cursor);
    case FLEXI_TYPE_VECTOR_SINT2: reader->typed_vector_sint(cursor->cursor, cursor->stride, 2); return true;
    case FLEXI_TYPE_VECTOR_UINT2: reader->typed_vector_uint(cursor->cursor, cursor->stride, 2); return true;
    case FLEXI_TYPE_VECTOR_FLOAT2: reader->typed_vector_flt(cursor->cursor, cursor->stride, 2); return true;
    case FLEXI_TYPE_VECTOR_SINT3: reader->typed_vector_sint(cursor->cursor, cursor->stride, 3); return true;
    case FLEXI_TYPE_VECTOR_UINT3: reader->typed_vector_uint(cursor->cursor, cursor->stride, 3); return true;
    case FLEXI_TYPE_VECTOR_FLOAT3: reader->typed_vector_flt(cursor->cursor, cursor->stride, 3); return true;
    case FLEXI_TYPE_VECTOR_SINT4: reader->typed_vector_sint(cursor->cursor, cursor->stride, 4); return true;
    case FLEXI_TYPE_VECTOR_UINT4: reader->typed_vector_uint(cursor->cursor, cursor->stride, 4); return true;
    case FLEXI_TYPE_VECTOR_FLOAT4: reader->typed_vector_flt(cursor->cursor, cursor->stride, 4); return true;
    case FLEXI_TYPE_BLOB: return reader_emit_vector_blob(reader, cursor);
    case FLEXI_TYPE_BOOL: reader->boolean(*(const bool *)(cursor->cursor)); return true;
...
bool flexi_write_inline_map(flexi_writer_s *writer, size_t len, flexi_width_e stride)
{
    ...
    // Write values.
    if (!write_vector_values(writer, len, stride_bytes, WRITE_VECTOR_ITER_FOR_MAP))
    {
        return false;
    }
...
bool flexi_write_finalize(flexi_writer_s *writer)
{
    ...
    case FLEXI_TYPE_UINT:
    case FLEXI_TYPE_FLOAT:
    case FLEXI_TYPE_BOOL: return false;
    case FLEXI_TYPE_MAP:
    case FLEXI_TYPE_VECTOR: {
